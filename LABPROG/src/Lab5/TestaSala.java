...
public class TestaSala {
	...
	public void testaCriaSala() {
		try {
			new Sala(-1, 10);
			Assert.fail("Esperava excecao, pois a quantidade de posicoes na horizontal (linhas) esta errada.");
		} catch (Exception e) {
			Assert.assertEquals(
					"Mensagem errada",
		...
		try {
			new Sala(10, -1);
			Assert.fail("Esperava excecao, pois a quantidade de posicoes na vertical (colunas) esta errada.");
		} catch (Exception e) {
			Assert.assertEquals(
					"Mensagem errada",
					"Parametros incorretos para a criacao da sala. As dimensoes da sala devem ser representadas por valores positivos maiores que zero",
		...
		try {
			new Sala(-1, -1);
			Assert.fail("Esperava excecao, pois a quantidade de posicoes na horizontal (linhas) e na vertical (colunas) esta errada.");
		} catch (Exception e) {
			Assert.assertEquals(
					"Mensagem errada",
					"Parametros incorretos para a criacao da sala. As dimensoes da sala devem ser representadas por valores positivos maiores que zero",
